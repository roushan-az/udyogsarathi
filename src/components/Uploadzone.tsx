...
import {
  ...
  CloudUpload, Loader2, Tag, X, AlertTriangle,
} from 'lucide-react'
import type { DocumentCategory, UploadState } from '../types'
import { CATEGORIES, CATEGORY_COLORS, CATEGORY_ICONS, formatFileSize } from '../utils'
...
export const UploadZone: React.FC = () => {
  ...
  const [state, setState] = useState<UploadState>({
    file: null, preview: null, category: 'Sales',
    tags: [], progress: 0, status: 'idle',
  })

  // ── Drop handler ────────────────────────────────────────────────────────────
  const onDrop = useCallback((accepted: File[], rejected: unknown[]) => {
    if ((rejected as File[]).length > 0) {
      toast.error('Invalid file. JPG, PNG, WebP, TIFF, BMP under 10 MB.')
      return
    }
    ...
    setState(prev => ({
      ...
      errorMessage: undefined, resultDocument: undefined,
    }))
  }, [])

  const { getRootProps, getInputProps, isDragActive, isDragReject } = useDropzone({
    onDrop, accept: ACCEPTED_TYPES, maxSize: MAX_SIZE, maxFiles: 1,
  })
  ...
  const handleUpload = async () => {
    ...
    try {
      /**
       * documentService.uploadDocument sends multipart/form-data to:
       *   POST /api/documents/upload
       *
       * The backend pipeline (FastAPI):
       *   1. Validates image type + size
       *   2. Converts image → PDF in memory (Pillow / img2pdf)
       *   3. Uploads PDF to Azure Blob Storage
       *   4. Opens PostgreSQL transaction → INSERT document + activity_log
       *   5. COMMIT on success   |   ROLLBACK + DELETE blob on failure
       *
       * onProgress fires as the multipart body travels to the server (0→100).
       * After 100 % the server-side pipeline still runs, so we clamp UI to 90
       * until we receive the 201 response, then jump to 100.
       */
      const response = await documentService.uploadDocument({
        ...
        onProgress: (pct: number) => {
          ...
          setState(prev => ({ ...prev, progress: Math.min(pct, 90) }))
        },
      })

      // Server finished — jump to 100
      setState(prev => ({ ...prev, progress: 100 }))

      if (response.success && response.document) {
        addDocument(response.document)
        setState(prev => ({ ...prev, status: 'success', resultDocument: response.document }))
        toast.success('Document uploaded · stored in Azure Blob · recorded in PostgreSQL ✓')
      } else {
      ...
      const msg =
        // FastAPI error body: { success: false, error: { message: '...' } }
      ...
      toast.error(`Upload failed — Azure transaction rolled back. ${msg}`)
    }
  }
          ...
          <AnimatePresence mode="wait">
            {state.status === 'success' ? (
              ...
              <motion.div key="dropzone" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  ...
                  {state.file ? (
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
                      ...
                      <button onClick={e => { e.stopPropagation(); handleReset() }}
                        style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 6, padding: '4px 12px', cursor: 'pointer' }}>
                        Change file
                      </button>
                    </div>
                  ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14 }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.35)', lineHeight: 1.6 }}>
                          JPG, PNG, WebP, TIFF, BMP · Max 10 MB<br />
                          Converted to PDF server-side via FastAPI
                        </div>
                      </div>
                    </div>
                  )}
                ...
                {/* Category grid */}
                <div style={{ background: 'rgba(21,34,64,0.8)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: '18px 18px', marginTop: 16 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>
                    {CATEGORIES.map(cat => {
                      ...
                      return (
                        <button key={cat} onClick={() => setState(prev => ({ ...prev, category: cat as DocumentCategory }))}
                          style={{ padding: '9px 8px', borderRadius: 10, cursor: 'pointer', transition: 'all 0.15s ease', border: `1px solid ${sel ? c.accent + '50' : 'rgba(255,255,255,0.07)'}`, background: sel ? c.bg : 'rgba(255,255,255,0.02)', color: sel ? c.text : 'rgba(255,255,255,0.5)', fontSize: 12, fontWeight: sel ? 600 : 400, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3 }}>
                          <span style={{ fontSize: 16 }}>{CATEGORY_ICONS[cat]}</span>{cat}
                        </button>
                      )
                    })}
                  </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ background: 'rgba(21,34,64,0.8)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: '16px 18px' }}>
            ...
            {[
              ...
              { step: '02', label: 'PDF conversion',      sub: 'In-memory, no temp files', orange: false },
              ...
              <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', marginBottom: i < 4 ? 10 : 0 }}>
                ...
                <div>
                  <div style={{ fontSize: 12, fontWeight: 500, color: 'rgba(255,255,255,0.7)' }}>{label}</div>
                ...
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 7 }}>
                    <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)' }}>
                      {state.progress < 90 ? 'Uploading to FastAPI…' : 'Server processing PDF…'}
                    </span>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6 }}>
                    {['Upload', 'Convert', 'Blob', 'DB'].map((s, i) => (
                      <span key={s} style={{ fontSize: 9.5, color: state.progress >= (i + 1) * 22 ? '#f97316' : 'rgba(255,255,255,0.2)', fontWeight: 600, letterSpacing: '0.04em', textTransform: 'uppercase' }}>{s}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              {state.status === 'error' && (
                <div style={{ padding: '10px 14px', borderRadius: 10, background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 11.5, color: 'rgba(255,255,255,0.4)', marginTop: 2 }}>{state.errorMessage || 'Transaction rolled back — no orphaned files.'}</div>
                  </div>
                </div>
              )}

              <motion.button
                whileHover={state.file && state.status !== 'uploading' ? { scale: 1.02 } : {}}
                whileTap={state.file && state.status !== 'uploading' ? { scale: 0.97 } : {}}
                onClick={handleUpload}
                disabled={!state.file || state.status === 'uploading'}
                style={{
                  width: '100%', height: 48, borderRadius: 12, border: 'none',
                  fontSize: 14, fontWeight: 700, cursor: (!state.file || state.status === 'uploading') ? 'not-allowed' : 'pointer',
                ...
                {state.status === 'uploading'
                  ...
                  : <><Upload size={16} /> Upload &amp; Convert to PDF</>}
              </motion.button>
            </div>
          )}
        </div>
...
const SuccessState: React.FC<{ doc: ReturnType<typeof Object.create>; onReset: () => void }> = ({ doc, onReset }) => (
  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
    ...
    <h3 style={{ fontSize: 'clamp(15px,3vw,18px)', fontWeight: 700, color: '#f0f4ff', marginBottom: 8 }}>
      Uploaded Successfully!
    </h3>
    <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)', marginBottom: 16, lineHeight: 1.6 }}>
      Image → PDF → Azure Blob → PostgreSQL ✓
    </p>
    {doc?.blobUrl && (
